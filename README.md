...
The experience is designed to feel like talking to a knowledgeable friend rather than filling out a compliance form, and works in eight languages, including full right-to-left support for Arabic and Urdu, so it can support creators internationally, not just in one market. The tool accepts common image formats including JPG, PNG, BMP, WEBP, and TIFF.
...
**Action layer.** Executes the selected outcome per face, drawing a live consent countdown badge that refreshes automatically every five seconds while a timer is still active, so reopening the saved image mid-countdown always shows an accurate remaining time rather than a frozen value. It applies the selected blur style using OpenCV image processing (pixelation, Gaussian blur, elliptical masking, or emoji compositing via Pillow). Every blur is applied to a bounding box expanded by fifteen percent in every direction beyond the detected face, so slightly rotated or off-angle detections are still fully covered. A background watcher thread tracks every active consent timer in true chronological expiry order and automatically re-renders the image with the correct blur applied the moment each timer expires, using the original stored face coordinates so the correct face is always targeted, and correctly restoring emoji overlays rather than losing them during re-renders.
...
- Diagnose and fix a bug where emoji blur was silently replaced with plain square pixelation whenever a different face's consent timer triggered a full image re-render, ensuring the originally chosen emoji is always restored.
- Diagnose and fix a timing bug where the expiry watcher could process faces out of true chronological order if the user took time answering consent questions, occasionally missing an expiry deadline by a few seconds; fixed to always sort by actual expiry time rather than consent duration alone.
- Add Dutch as an eighth supported language, fully translated and tested end-to-end including the PDF report, bringing the total to eight languages covering major European, Middle Eastern, and South Asian markets.
...
- Extend translation coverage across all eight supported languages so the compliance badge, blur style names, and PDF outcome labels display correctly in every language, and shorten PDF column headers that overflowed in several languages.
